...
namespace App\Console\Commands\MyCrudGenerator\Commands;
 ...
 */
class MyCrudGenerator extends GeneratorCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'make:my-crud
                            {name : Table name}
     ...
     * @throws FileNotFoundException
     */
    protected function copyBaseTemplateFiles(): static
    {
        $this->info('Copy BaseRequest to Requests folder ...');
        $baseRequestTemplatePath = $this->getBaseTemplatePath('BaseFormTemplateRequest');
        $destinationPath = $this->_getRequestPath('BaseForm');

        if ($this->files->exists($destinationPath) && $this->ask('Already exist BaseFormRequest. Do you want overwrite (y/n)?', 'n') == 'n') {
            return $this;
        }

        $this->copy($baseRequestTemplatePath, $destinationPath);
     ...
     *
     */
    protected function buildRequest(): static
    {
        $this->info('Creating Store Requests Class ...');

        // Make StoreRequest Class
        $storeRequestPath = $this->_getStoreRequestPath($this->name);
        if ($this->files->exists($storeRequestPath) && $this->ask('Already exist Store Request. Do you want overwrite (y/n)?', 'y') == 'n') {
            return $this;
        }
        ...
        $requestTemplate = str_replace(
            array_keys($replace), array_values($replace), $this->getStub('StoreRequest')
        );
        $this->write($storeRequestPath, $requestTemplate);

        $updateRequestPath = $this->_getUpdateRequestPath($this->name);
        if ($this->files->exists($updateRequestPath) && $this->ask('Already exist Update Request. Do you want overwrite (y/n)?', 'y') == 'n') {
            return $this;
        }
        $this->info('Creating Update Requests Class ...');
        // Make UpdateRequest Class
        $requestTemplate = str_replace(
            array_keys($replace), array_values($replace), $this->getStub('UpdateRequest')
        );
        $this->write($updateRequestPath, $requestTemplate);
    ...
    protected function buildPolicy(): static
    {
        ...
        if ($this->files->exists($policyPath) && $this->ask('Already exist Policy. Do you want overwrite (y/n)?', 'y') == 'n') {
            return $this;
        }

        $this->info('Creating Policy Class...');
        ...
        $this->write($policyPath, $requestTemplate);
