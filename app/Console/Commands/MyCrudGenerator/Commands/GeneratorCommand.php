...
use App\Console\Commands\MyCrudGenerator\ModelGenerator;
use Exception;
...
abstract class GeneratorCommand extends Command implements PromptsForMissingInput
{
    ...
    public function __construct(Filesystem $files)
    {
        ...
        $this->policyNamespace = config('crud.policy', $this->policyNamespace);
    }
     ...
     * @param $content
     */
    protected function copy($from, $to): void
    {
        $this->makeDirectory($from);
    ...
    {
        $stub_path = config('my-crud.stub_path', 'default');
     ...
     * @throws FileNotFoundException
     */
    protected function getBaseTemplatePath(string $type, bool $content = false): string
    {
        $stub_path = config('my-crud.stub_path', 'default');
        ...
        $path = Str::finish($stub_path, 'BaseTemplate/')."$type.php";
     ...
     * @return string
     */
    protected function _getStoreRequestPath($name): string
    {
        return app_path($this->_getNamespacePath($this->requestNamespace)."Store{$name}Request.php");
    }
     ...
     * @return string
     */
    protected function _getUpdateRequestPath($name): string
    {
        return app_path($this->_getNamespacePath($this->requestNamespace)."Update{$name}Request.php");
    }
    ...
    {
        return app_path($this->_getNamespacePath($this->policyNamespace)."{$name}Policy.php");
    }
